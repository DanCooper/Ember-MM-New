...
#Region "Methods"
    ...
    Public Sub GetEpFolderContents(ByRef Episode As EpisodeContainer)
        ...
        Try
            ...
            If String.IsNullOrEmpty(Episode.Poster) AndAlso AdvancedSettings.GetBooleanSetting("YAMJEpisodePoster", False, "multi.Compatibility") Then
                fName = String.Concat(tmpName, ".videoimage.jpg")
                Episode.Poster = fList.FirstOrDefault(Function(s) s.ToLower = fName.ToLower)
            End If
    ...
    Public Sub GetMovieFolderContents(ByRef Movie As MovieContainer)
        ...
        Try
            If Master.eSettings.VideoTSParent AndAlso FileUtils.Common.isVideoTS(Movie.Filename) Then
                ...
                End If
            ElseIf Master.eSettings.VideoTSParentXBMC AndAlso FileUtils.Common.isVideoTS(Movie.Filename) Then
                isXBMC = True
            ...
            ElseIf Master.eSettings.VideoTSParent AndAlso FileUtils.Common.isBDRip(Movie.Filename) Then
                ...
                If Movie.isSingle AndAlso File.Exists(String.Concat(Directory.GetParent(Directory.GetParent(Movie.Filename).FullName).FullName, Path.DirectorySeparatorChar, "extrathumbs", Path.DirectorySeparatorChar, "thumb1.jpg")) Then
                    Movie.Extra = String.Concat(Directory.GetParent(Directory.GetParent(Movie.Filename).FullName).FullName, Path.DirectorySeparatorChar, "extrathumbs", Path.DirectorySeparatorChar, "thumb1.jpg")
                End If
            ElseIf Master.eSettings.VideoTSParentXBMC AndAlso FileUtils.Common.isBDRip(Movie.Filename) Then
                isXBMC = True
			...
			For Each fFile As String In fList
                ...
                If String.IsNullOrEmpty(Movie.Poster) Then
                    If (Movie.isSingle AndAlso (Master.eSettings.MovieTBN AndAlso fFile.ToLower = Path.Combine(parPath, "movie.tbn")) _
                                ...
                                OrElse (Master.eSettings.MovieNameDashPosterJPG AndAlso Not isXBMC AndAlso (fFile.ToLower = Path.Combine(parPath, "video_ts-poster.jpg") OrElse (fFile.ToLower = Path.Combine(parPath, "poster.jpg") AndAlso Movie.Filename.ToLower = String.Concat(parPath, "\", "video_ts.ifo") OrElse Movie.Filename.ToLower = String.Concat(parPath, "\", "00000.m2ts"))))) _
    ...
    Public Sub GetSeasonImages(ByRef TVDB As Structures.DBTV, ByVal sSeason As Integer)
        ...
        Try
            ...
            If AdvancedSettings.GetBooleanSetting("YAMJTVImageNaming", False, "multi.Compatibility") Then

                Try
                    lFiles.AddRange(Directory.GetFiles(ShowPath, "*.jpg"))
                Catch
                End Try
            ...
            If bInside AndAlso ((Master.eSettings.SeasonPosterTBN OrElse Master.eSettings.SeasonPosterJPG OrElse _
                                ...
                                Master.eSettings.SeasonFolderJPG OrElse AdvancedSettings.GetBooleanSetting("YAMJSeasonPoster", False, "multi.Compatibility") AndAlso _
                                String.IsNullOrEmpty(TVDB.SeasonPosterPath)) OrElse _
                                (Master.eSettings.SeasonFanartJPG OrElse Master.eSettings.SeasonDashFanart OrElse _
                                 Master.eSettings.SeasonDotFanart OrElse AdvancedSettings.GetBooleanSetting("YAMJSeasonFanart", False, "multi.Compatibility") AndAlso _
    ...
    Public Sub GetShowFolderContents(ByRef tShow As TVShowContainer, Optional ByVal ID As Long = 0)
        ...
        Try
            ...
            If String.IsNullOrEmpty(tShow.AllSeasonPoster) AndAlso AdvancedSettings.GetBooleanSetting("YAMJAllSeasonPoster", False, "multi.Compatibility") Then
                Dim tPath As String = String.Empty
                Dim seasonPath As String = String.Empty
                If tShow.Episodes.Count > 0 Then
                    seasonPath = Directory.GetParent(tShow.Episodes.FirstOrDefault.Filename).FullName
                ...
                End Try
                tPath = Path.Combine(parPath, seasonPath)
                fName = Path.Combine(tPath, String.Concat("Set_", FileUtils.Common.GetDirectory(parPath), "_1.banner.jpg"))
                tShow.AllSeasonPoster = fList.FirstOrDefault(Function(s) s.ToLower = fName.ToLower)
            End If
